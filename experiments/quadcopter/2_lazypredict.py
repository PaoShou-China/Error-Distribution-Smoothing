...
def load_data(features_path, labels_path):
    ...
    features = np.loadtxt('data/processed/4/' + features_path)
    labels = np.loadtxt('data/processed/4/' + labels_path)
...
def evaluate_with_lazypredict(train_features, train_labels, test_features, test_labels, dataset_name, dim_index=None):
    ...
    for model_name, model in reg.models.items():
        try:
            ...
            # Print partial predictions and max error
            print(f"\nModel: {model_name}")
    ...
    output_file = f'results/lazypredict/quadcopter/{dataset_name.split("_")[0]}_dim{dim_index}.csv'
...
if __name__ == "__main__":
    datasets = [
        ...
        ('sampled_features_dim0', 'sampled_labels_dim0', 0),
    ]
