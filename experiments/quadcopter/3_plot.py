...
def plot_comparison(all_results, metric, title, ylabel):
    """Plot comparison of model metrics across different datasets (style consistent with cartpole/dim0, legend on right)"""
    ...
    plt.tight_layout()
    # save_data_path = f'4/quadcopter/visualization/{metric.lower().replace(" ", "_")}_comparison.txt'
    # save_data_to_txt(all_results, metric, dataset_order, models, save_data_path)
    save_path = f'results/figures/4/{metric.lower().replace(" ", "_")}_comparison.pdf'
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()

def plot_time_comparison(all_results):
    """Plot model training time comparison (style consistent with cartpole/dim0, legend on right)"""
    plt.style.use('seaborn-v0_8-whitegrid')
    plt.rcParams.update({
        'font.size': 12,
        'axes.labelsize': 14,
    ...
    ax.legend(loc='best', frameon=True, fancybox=True, shadow=True)
    plt.tight_layout()
    # save_data_path = '4/quadcopter/visualization/training_time_comparison.txt'
    # save_data_to_txt(all_results, 'Time Taken', dataset_order, models, save_data_path)
    save_path = 'results/figures/4/training_time_comparison_dim0.pdf'
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Define list of models to display (consistent with cartpole/dim0)
    selected_models = [
        'Ridge',
        'BayesianRidge',
    ...
    results_files = {
        'Original': 'results/lazypredict/quadcopter/train_dim0.csv',
        'SMOGN': 'results/lazypredict/quadcopter/smogn_dim0.csv',
        'Sampled': 'results/lazypredict/quadcopter/sampled_dim0.csv',
        'EDS': 'results/lazypredict/quadcopter/eds_dim0.csv'
    }
    ...
    print("Visualization plots and data have been saved to the 'visualization' directory.")
