...
def plot_comparison(all_results, metric, title, ylabel):
    """Plot comparison of model metrics across different datasets"""
    ...
    plt.tight_layout()
    # save_data_path = f'results/cartpole/visualization/{metric.lower().replace(" ", "_")}_comparison_dim0.txt'
    # save_data_to_txt(all_results, metric, dataset_order, models, save_data_path)
    save_path = f'results/figures/3/{metric.lower().replace(" ", "_")}_comparison_dim0.pdf'
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()

def plot_time_comparison(all_results):
    """Plot model training time comparison (four datasets, consistent style and color)"""
    plt.style.use('seaborn-v0_8-whitegrid')
    plt.rcParams.update({
        'font.size': 12,
        'axes.labelsize': 14,
    ...
    ax.legend(loc='best', frameon=True, fancybox=True, shadow=True)
    plt.tight_layout()
    # save_data_path = '4/cartpole/visualization/training_time_comparison_dim0.txt'
    # save_data_to_txt(all_results, 'Time Taken', dataset_order, models, save_data_path)
    save_path = 'results/figures/3/training_time_comparison_dim0.pdf'
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Define list of models to display
    selected_models = [
        'Ridge',
        'BayesianRidge',
    ...
    # Define result file paths (order matters)
    results_files = {
        'Original': 'results/lazypredict/cartpole/train_dim0.csv',
        'SMOGN': 'results/lazypredict/cartpole/smogn_dim0.csv',
        'Sampled': 'results/lazypredict/cartpole/sampled_dim0.csv',
        'EDS': 'results/lazypredict/cartpole/eds_dim0.csv'
    }
    # Load all results
    all_results = {}
