...
def evaluate_mlp(train_features, train_labels, test_features, test_labels, max_epochs=2000, eval_interval=10,
    ...
    # Ensure label data is a 2D array, even if there's only one output variable
    ...
    for epoch in range(0, max_epochs, eval_interval):
        ...
        # Calculate the composite error (Euclidean distance) for each sample
        errors = np.sqrt(np.sum((y_pred - test_labels) ** 2, axis=1))
...
# Function to read evaluation results and plot them
