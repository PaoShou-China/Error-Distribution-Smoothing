...
import numpy as np
import pandas as pd  # For simple moving average
...
def plot_evaluation_results(files, labels, title_prefix, smoothing_method='moving_average', window_size=40, alpha=0.6):
    ...
    for ax in axes:
        ...
        ax.tick_params(axis='both', which='minor', labelsize=font_size['ticks'] - 2)
    ...
    for file, label, color in zip(files, labels, colors):
        ...
        axes[0].semilogy(epochs, y_smooth_rmse, label=label, color=color, linewidth=3)
        ...
        axes[1].semilogy(epochs, y_smooth_max_error, label=label, color=color, linewidth=3)
